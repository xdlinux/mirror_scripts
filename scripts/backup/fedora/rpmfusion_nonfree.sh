...
REPONAME="rpmfusion_nonfree"
SYNC_LOGS="$SYNC_HOME/logs/fedora"
SYNC_FILES="/srv/ftp/fedora/rpmfusion/nonfree/release/13/Everything"
...
SYNC_SERVER=rsync://mirror.yandex.ru/fedora/rpmfusion/nonfree/fedora/releases/13/Everything
# Set the format of the log file name
# This example will output something like this: sync_20070201-8.log
LOG_FILE="$REPONAME.release_$(date +%Y%m%d-%H).log"
...
echo ">> ---" >> "$SYNC_LOGS/$LOG_FILE"
  ...
  for repo in ${SYNC_REPO[@]}; do
    ...
    rsync -avrz --delete-after --exclude=debug/ $SYNC_SERVER/$repo "$SYNC_FILES" >> "$SYNC_LOGS/$LOG_FILE" 
