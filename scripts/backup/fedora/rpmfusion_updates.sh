...
# This example will output something like this: sync_20070201-8.log
	LOG_FILE="$REPONAME-$(date +%Y%m%d-%H).log"
...
for RE in ${SYNC_RE[@]}; do
	SYNC_FILES="/srv/ftp/fedora/rpmfusion/$RE/updates/13"
...
#rsync -rptlv --safe-links --delete-after-after --delay-updates $SYNC_SERVER/$repo "$SYNC_FILES" >> "$SYNC_LOGS/$LOG_FILE"
	SYNC_SERVER=rsync://mirror.yandex.ru/fedora/rpmfusion/$RE/fedora/updates/13
