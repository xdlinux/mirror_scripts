...
SYNC_FILES="/srv/ftp/fedora/updates/14"
SYNC_LOCK="$SYNC_HOME/fedora_updates_14.lck"
...
#SYNC_SERVER=rsync://mirror6.bjtu.edu.cn/fedora/updates/14
SYNC_SERVER=rsync://mirrors6.ustc.edu.cn/fedora-enchilada/updates/14/
# Set the format of the log file name
# This example will output something like this: sync_20070201-8.log
LOG_FILE="updates_14_$(date +%Y%m%d-%H).log"
...
echo ">> ---" >> "$SYNC_LOGS/$LOG_FILE"
  ...
  for repo in ${SYNC_REPO[@]}; do
    ...
    rsync -6 -avr --delete-after $SYNC_SERVER/$repo "$SYNC_FILES" >> "$SYNC_LOGS/$LOG_FILE" &
