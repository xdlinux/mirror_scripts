...
SYNC_FILES="/var/ftp/archlinux/community/os"
SYNC_LOCK="$SYNC_HOME/mirrorsync_comm_any.lck"
...
#SYNC_SERVER=rsync://mirror.rit.edu/archlinux/community/os
SYNC_SERVER=rsync://oss.ustc.edu.cn/pub/archlinux/community/os
...
LOG_FILE="pkgsync_community_any_$(date +%Y%m%d-%H).log"
...
if [ -z $SYNC_REPO ]; then
  ...
  rsync -rptLv --delete-after --delay-updates --exculde "*.~tmp~*" $SYNC_SERVER "$SYNC_FILES" >> "$SYNC_LOGS/$LOG_FILE"
else
  ...
  for repo in ${SYNC_REPO[@]}; do
    ...
    #
    rsync -avz --delete-after --exculde *.~tmp~* $SYNC_SERVER/$repo "$SYNC_FILES" >> "$SYNC_LOGS/$LOG_FILE"
