...
SYNC_FILES="/var/ftp/archlinux/extra/os/"
SYNC_LOCK="$SYNC_HOME/mirrorsync_extra_x64.lck"
...
SYNC_REPO=(any i686 x86_64)

# Set the rsync server to use
# Only official public mirrors are allowed to use rsync.archlinux.org
#SYNC_SERVER=rsync://mirror.yandex.ru/archlinux/extra/os
SYNC_SERVER=rsync://oss.ustc.edu.cn/pub/archlinux/extra/os
#SYNC_SERVER=darkstar.ist.utl.pt::pub/archlinux/extra/os/
...
LOG_FILE="pkgsync_extra_x64_$(date +%Y%m%d-%H).log"
...
if [ -z $SYNC_REPO ]; then
  ...
  rsync -rptLv --delete-after --delay-updates --exclude *.~tmp~/ $SYNC_SERVER "$SYNC_FILES" >> "$SYNC_LOGS/$LOG_FILE"
else
  ...
  for repo in ${SYNC_REPO[@]}; do
    ...
    #
    rsync -avz --delete-after --exclude *.~tmp~/ $SYNC_SERVER/$repo "$SYNC_FILES" >> "$SYNC_LOGS/$LOG_FILE"
