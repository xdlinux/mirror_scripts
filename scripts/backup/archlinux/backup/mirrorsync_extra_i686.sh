...
SYNC_LOCK="$SYNC_HOME/mirrorsync_extra_i686.lck"
...
LOG_FILE="pkgsync_extra_i686_$(date +%Y%m%d-%H).log"
...
if [ -z $SYNC_REPO ]; then
  ...
  for repo in ${SYNC_REPO[@]}; do
    ...
    #
    rsync -avz --delete-after  $SYNC_SERVER/$repo "$SYNC_FILES" >> "$SYNC_LOGS/$LOG_FILE"
